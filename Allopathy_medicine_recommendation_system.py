...
# Get diseases from the user
input_diseases = input("Enter diseases (comma-separated): ").strip().split(',')

# Get recommendations for input diseases
recommendations = recommendation_system.get_recommendations(input_diseases)

# Print recommendations for input diseases
for i, disease in enumerate(input_diseases):
    print("Top recommendations for", disease, ":", [list(medication_diseases.keys())[j] for j in recommendations[:, i]])
